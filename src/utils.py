...
def clamp_point(point: Point, width: int, height: int) -> Point:
    """Keep a point within the frame boundaries."""
...
def point_in_rect(point: Point, rect: Tuple[int, int, int, int]) -> bool:
...
def shrink_rect(rect: Tuple[int, int, int, int], inset: int) -> Tuple[int, int, int, int]:
...
def build_save_path(prefix: str = "air_canvas", extension: str = "png") -> str:
